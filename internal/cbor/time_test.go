...
var timeIntegerTestcases = []struct {
	...
	{"Feb 3, 2013 at 7:54pm (PST)", "\xc1\x1a\x51\x0f\x30\xd8"},
	{"Feb 3, 1950 at 7:54pm (PST)", "\xc1\x3a\x25\x71\x93\xa7"},
}
...
func TestAppendTimePastPresentInteger(t *testing.T) {
	...
	for _, tt := range timeIntegerTestcases {
		tin, err := time.Parse(timeIntegerFmt, tt.in)
		if err != nil {
			fmt.Println("Cannot parse input", tt.in, ".. Skipping!")
			continue
		}
		b := AppendTime([]byte{}, tin)
		if got, want := string(b), tt.out; got != want {
			t.Errorf("appendString(%s) = 0x%s, want 0x%s", tt.in,
...
var timeFloatTestcases = []struct {
	...
	{"2006-01-02T15:04:05.999999-08:00", "\xc1\xfb\x00\x41\xd0\xee\x6c\x59\x7f\xff"},
	{"1956-01-02T15:04:05.999999-08:00", "\xc1\xfb\x00\xc1\xba\x53\x81\x1a\x00\x00"},
}
...
func TestAppendTimePastPresentFloat(t *testing.T) {
	...
	for _, tt := range timeFloatTestcases {
		tin, err := time.Parse(timeFloatFmt, tt.in)
		if err != nil {
			fmt.Println("Cannot parse input", tt.in, ".. Skipping!")
			continue
		}
